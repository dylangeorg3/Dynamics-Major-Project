...
# Fixed TopCrankSlider class with correct constraints
class TopCrankSlider:
    ...
    def _setup_constraints(self):
        ...
        # The top end of link 6 should be at a fixed position
        fixed_height = 5.0  # Adjust this based on your desired system height
        constraint_1 = x_com_6 + R(self.theta4) @ sp.Matrix([L6/2, 0]) - sp.Matrix([0, fixed_height])
        C1 = constraint_1.dot(i_cap)
        C2 = constraint_1.dot(j_cap)
        
        # FIXED: Link 6 to Link 5 connection (pin joint)
        # Connect the bottom of link 6 to the top of link 5
        constraint_2 = (x_com_6 + R(self.theta4) @ sp.Matrix([-L6/2, 0]) - 
                       x_com_5 - R(self.theta3) @ sp.Matrix([L5/2, 0]))
        C3 = constraint_2.dot(i_cap)
        C4 = constraint_2.dot(j_cap)
        
        # FIXED: Link 5 to piston connection
        # Connect the bottom of link 5 to the top piston
    ...
    def get_force_vector(self, q_vals, dq_vals, spring_force):
        ...
        Q = np.array([
            0, -F0 * np.cos(q_vals[4]) - m4 * g + spring_force,  # piston 4 + spring (note: using theta3 index 4)
            0, -m5*g, 0,                                         # body 5
...
# Also need to fix the initial conditions for the top system
def get_initial_conditions_fixed(self):
    """Calculate initial conditions for both systems with correct top system setup"""
    ...
    # FIXED: Top system initial positions
    # Work backwards from the fixed top point
    fixed_height = 5.0
    
    # Link 6 center position (fixed top end at [0, fixed_height])
    x6_init = 0
    y6_init = fixed_height - L6/2
    theta4_init = -np.pi/2  # Pointing downward
    
    # Link 5 center position (connected to bottom of link 6)
    x5_init = 0  
    y5_init = y6_init - L6/2 - L5/2
    theta3_init = -np.pi/2  # Pointing downward
    
    # Top piston position (connected to bottom of link 5)
    x4_init = 0
    y4_init = y5_init - L5/2
